...
<div class='flexwrap justify-content'>  
    ...
    <li>
        ...
        <p style="margin-top: 10px;font-size: 13px;">{{$pinnedThread->replies}} Replies {{$pinnedThread->files}} Files  
            @if(isset(auth()->user()->username)) <img onclick="window.location='{{route('moderation.files.spoiler',['reply'=>$pinnedThread->thread_id,'thread'=>$pinnedThread->thread_id] )}}';" class='icons' title='Mark as spoiler' src="{{asset('files/system/spoiler.png')}}"> <img class='icons' title='Pin/Un-Pin' onclick="window.location='{{route('moderation.pin',$pinnedThread->thread_id)}}';" src="{{asset('files/system/pushpin.png')}}"> <img class='icons' onclick="window.location='{{route('moderation.archive',$pinnedThread->thread_id)}}';" title='Archive/Un-Archive' src="{{asset('files/system/archive.png')}}"> <img class='icons' onclick="window.location='{{route('moderation.thread.delete',$pinnedThread->id)}}';" title='Delete' src="{{asset('files/system/delete.png')}}"> <img class='icons' onclick="window.location='{{route('moderation.thread.ban',$pinnedThread->id)}}';" title='Delete & Ban IP' src="{{asset('files/system/ban.png')}}">@endif 
        </p>
    </li>
    <br>
    </ol>
